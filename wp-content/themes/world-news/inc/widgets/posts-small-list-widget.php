<?php
if ( ! class_exists( 'World_News_Posts_Small_List_Widget' ) ) {
	/**
	 * Adds World_News_Posts_Small_List_Widget Widget.
	 */
	class World_News_Posts_Small_List_Widget extends WP_Widget {

		/**
		 * Register widget with WordPress.
		 */
		public function __construct() {
			$world_news_list_widget_ops = array(
				'classname'   => 'ascendoor-widget magazine-small-list-section style-1',
				'description' => __( 'Retrive Posts List Widgets', 'world-news' ),
			);
			parent::__construct(
				'world_news_magazine_small_list_widget',
				__( 'Ascendoor Posts Small List Widget', 'world-news' ),
				$world_news_list_widget_ops
			);
		}
		...
		public function widget( $args, $instance ) {
			...
			$posts_list_post_count   = isset( $instance['number'] ) ? absint( $instance['number'] ) : 6;
		...
		public function form( $instance ) {
			...
			<p>
				<label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>"><?php esc_html_e( 'Number of posts to show (Min: 1 | Max: 6):', 'world-news' ); ?></label>
				<input class="tiny-text" id="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'number' ) ); ?>" type="number" step="1" min="1" max="6" value="<?php echo absint( $posts_list_post_count ); ?>" size="3" />
			</p>
			<p>
